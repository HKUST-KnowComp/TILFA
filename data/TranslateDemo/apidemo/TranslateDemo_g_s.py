...
stance = df.stance.value_counts().to_dict()

# Get max intent count to match other minority classes through data augmentation
import operator
max_intent_count = max(stance.items(), key=operator.itemgetter(1))[1]
...
newdf = pd.DataFrame()
for stance_i, count in stance.items():
    ...
    if (multiplication_count):
        ...
        orig_augment_df = df[df["stance"] == stance_i]
        for idx in tqdm.tqdm(orig_augment_df.index):
            # Extracting existing minority class batch
            tweet = orig_augment_df.loc[idx]
            tweet_text = tweet["tweet_text"]
            # dummy1 = pd.DataFrame([tweet_text], columns=['tweet_text'])
            # dummy1["stance"] = stance_i
        ...
        newdf = newdf.append(df[df["stance"] == stance_i])

# Print count of all new data points
newdf.stance.value_counts()

# Save newdf back to file
newdf.to_csv('../../gun_control_dev_after_stance_augmentation.csv')
