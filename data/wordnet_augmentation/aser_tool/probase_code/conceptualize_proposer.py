from probase_processor import ProbaseConcept
...
class Proposer:
    def __init__(self, probase_path):
        self._probase = ProbaseConcept(probase_path)
        all_entities = set(self._probase.concept2idx.keys()).union(set(self._probase.instance2idx.keys()))
        entity_freqs = []
        concept_freqs = []
        inst_freqs = []
        inst_cnts = []
        self._infl = inflect.engine()
        co_occurrence_sum = 0
        ...
        self.co_occurrence_sum = co_occurrence_sum
        self.inst_cnts = inst_cnts

        self.concept_freq_map = dict(concept_freqs)
        self.inst_freq_map = dict(inst_freqs)
        self.inst_cnt_map = dict(inst_cnts)
    ...
    def substitute(self, pre_text, cand, post_text, det_idx, det_text, alt_dets):
        if not alt_dets:
            if det_text in ['a', 'an'] and expr != 'aser':
                possible_dets = [self._infl.an(pre_text[det_idx + len(det_text) + 1:] + cand + post_text).split(' ')[0]]
            else:
    ...
    def collect_substitution(self, text: str, concepts: List[EntityMentionE],
        ...
        for i, mention in enumerate(concepts):
            ...
            for cand, w in candidates:
                cand = self.remove_det(cand)
                if cand == mention.base_concept:
                    continue
                for _text in self.substitute(pre_text, cand, post_text, mention.det_idx, mention.det_text, try_alt_dets):
...
if __name__ == '__main__':
    ...
    parser.add_argument("--probase_path", type=str,
                        default="/home/data/zwanggy/probase/data-concept-instance-relations.txt")
