...
def main():
    ...
    exp_name = get_exp_name_layout_alltrain(args)
    ...
    for dataset_name in ['gun_control', 'abortion']:
        ...
        dataset_cache_name = get_dataset_cache_name_layout_alltrain(model_args, data_args, dataset_name, data_args.exp_mode)
        if os.path.exists(dataset_cache_name) and not data_args.overwrite_cache:
            ...
            # df_test = pd.read_csv(os.path.join(data_args.data_dir, dataset_name + '_test.csv'), index_col=0)
