...
def predict(model, test_dataloaders, criterion, output_name):
    ...
    with torch.no_grad():
        for i, (text_ids, input_ids, attention_masks, image) in enumerate(test_dataloaders):
...
if __name__ == '__main__':
    ...
    model_name = 'stance_alltrain_multimodality_resnet50_deberta-v3-large_lr1e-05_bs8_augmentation_wordnet1_pooler0_pure'
    output_name = 'stance_alltrain_multimodality_resnet50_deberta-v3-large_lr1e-05_bs8_augmentation_wordnet1_pooler0_pure_abortion.csv'
    ...
    test_dataset = ImageTextTestDataset(args, annotation=test_annotation, root_dir=os.path.join(args.data_dir, f'images/{dataset_name}'), transform=val_transform)
    # test_dataset = ImageTextDataset(args, annotation=test_annotation, root_dir=os.path.join(args.data_dir, f'images/{dataset_name}'), transform=val_transform)
    ...
    if args.img_model == 0:
        model1 = MultiModelResnet50(args=args, text_model_name=args.text_model_name, out_dim=1, freeze_model=args.freeze_model)
    elif args.img_model == 1:
        model1 = MultiModelResnet101(args=args, text_model_name=args.text_model_name, out_dim=1, freeze_model=args.freeze_model)
    else:
        model1 = MultiModelVGG16(args=args, text_model_name=args.text_model_name, out_dim=1, freeze_model=args.freeze_model)
