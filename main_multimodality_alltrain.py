...
if __name__ == '__main__':
    ...
    if args.img_model == 0:
        init_model = MultiModelResnet50(args=args, text_model_name=args.text_model_name, out_dim=1, freeze_model=args.freeze_model)
    elif args.img_model == 1:
        init_model = MultiModelResnet101(args=args, text_model_name=args.text_model_name, out_dim=1, freeze_model=args.freeze_model)
    else:
        init_model = MultiModelVGG16(args=args, text_model_name=args.text_model_name, out_dim=1, freeze_model=args.freeze_model)
    ...
    for dataset_name in ['gun_control', 'abortion']:
        ...
        if args.exp_mode == 0:  # stance
            if args.use_wordnet == 0:
                df = pd.read_csv(os.path.join(args.data_dir, dataset_name + '_train_after_stance_augmentation_new.csv'), index_col=0)
            else:
                df = pd.read_csv(os.path.join(args.data_dir, dataset_name + '_train_after_stance_augmentation_wordnet.csv'), index_col=0)
        else:
            if args.use_wordnet == 0:
                df = pd.read_csv(os.path.join(args.data_dir, dataset_name + '_train_after_persuasiveness_augmentation_new.csv'), index_col=0)
            else:
                df = pd.read_csv(os.path.join(args.data_dir, dataset_name + '_train_after_persuasiveness_augmentation_wordnet.csv'), index_col=0)
        ...
        val_dataloaders = DataLoader(val_dataset, collate_fn=collate_fn, batch_size=args.batch_size, drop_last=True)
        ...
        train_dataloaders = DataLoader(train_dataset, collate_fn=collate_fn, batch_size=args.batch_size, drop_last=True)
